...
public class QueryManager extends MySQLManager {
	...
	public static Equipment loadEquipment(int playerID, int config) {
		...
		return new Equipment(playerID, config, B02, B01, A03, A02, A01, G3N79, G3N69, G3N33, G3N32, G3N20, G3N10, LF3, LF2, MP1, LF1);
	}
	
	/**
	 * Esta funcion es una puta mierda.
	 * TODO reescribir esto de una manera mas comoda pls
	 * @param packet
	 * @return
	 */
	public static void checkObject(String packet) {
		...
		//Si el usuario ya tenia un 'equipamiento'
		if(GameManager.equipment.containsKey(playerID + "|" + config)) {
			Equipment playerEquipment = GameManager.getEquipment(playerID + "|" + config);
			//Lasers, generators, etc..
			switch(p[2]) {
				case "lasers":
					playerEquipment.updateLasers(LF3, LF2, MP1, LF1);
					break;
				case "generators":
					playerEquipment.updateGenerators(B02, B01, A03, A02, A01, G3N79, G3N69, G3N33, G3N32, G3N20, G3N10);
					break;
			}
			//Borro el antiguo y cargo el nuevo en memoria
			GameManager.equipment.remove(playerEquipment.getIdentifier());
			GameManager.addEquipment(playerEquipment);
		} else {
			GameManager.addEquipment(new Equipment(playerID, config, B02, B01, A03, A02, A01, G3N79, G3N69, G3N33, G3N32, G3N20, G3N10, LF3, LF2, MP1, LF1));
		}
	}
